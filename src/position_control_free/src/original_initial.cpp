...
Target_point.color.a = 1.0;
  ...
  Target_point.pose.position.z  = -(((target_position_z[0]-269))/3.5)/10;//((position_side_final[i][2]/4.08)-(457/4.08))*0.1;
...
Target_point.color.a = 1.0;
  ...
  Target_point.pose.position.z  = -(((target_position_z[1]-269))/3.5)/10;//((position_side_final[i][2]/4.08)-(457/4.08))*0.1;
...
for(int h = 0; h <27 ;h++){
  ...
  Target_point.pose.position.y  = -(((target_position_y[h])-410)/4.0)/10 ; 
  Target_point.pose.position.z  = -(((target_position_z[h]-269))/3.5)/10;//((position_side_final[i][2]/4.08)-(457/4.08))*0.1;
...
target_z = -(target_position_z[0]-269)/3.5;
...
//// position in mm
 ...
 if((mm_px)<5)  {mm_px = abs(side_f_3b-478)/4.0; //position_side_final[i][3] and position_side_final[i][1]
       ...
       mm_py = -((side_f_0b)-410)/4.0;
       mm_pz = -((side_f_2b-269))/3.5;//((position_side_final[i][2]/4.08)-(457/4.08))*0.1;
...
if((pixel_px)<5)  {pixel_px = abs(side_f_3b-478); //position_side_final[i][3] and position_side_final[i][1]
      ...
      pixel_py = -((side_f_0b)-410);
      pixel_pz = -((side_f_2b-269));//((position_side_final[i][2]/4.08)-(457/4.08))*0.1;
...
Target_point.color.a = 1.0;
  ...
  Target_point.pose.position.y  = -(((target_position_y[tnum])-416)/4.0)/10 ; 
  Target_point.pose.position.z  = -(((target_position_z[tnum]-282))/3.5)/10;//((position_side_final[i][2]/4.08)-(457/4.08))*0.1;
...
break;
	...
	if(mm_py>=0){
	   ...
	   dteny1 = (0.06*(target_y - mm_py))+tension_v1;
	
	if(dteny1 < 0){ dteny1 = 0.06;}

		dteny2 = 0.00;

	if((tension_v1 < 0.2)&&(dteny1 < 0.07)){
		dteny2 = 0.50;
		dteny1 = 0.00;		
		}
	...
	if(mm_py<=0){
	   ...
	   dteny2 = (0.06*(abs(target_y) - abs(mm_py) ))+tension_v2;

	if(dteny2 < 0){ dteny2 = 0.06;}

		dteny1 = 0.00;

	if((tension_v2 < 0.2)&&(dteny2 < 0.07)){
		dteny1 = 0.50;
		dteny2 = 0.00;		
		}
}
	...
	if(mm_pz>=0){
	   ...
	   dtenz1 = (0.06*(target_z - mm_pz))+tension_v3;
	
	if(dtenz1 < 0){ dtenz1 = 0.06;}

		dtenz2 = 0.00;

	if((tension_v3 < 0.2)&&(dtenz1 < 0.07)){
		dtenz2 = 0.50;
		dtenz1 = 0.00;		
		}
}
	...
	if(mm_pz<=0){
	   ...
	   dtenz2 = (0.06*(abs(target_z) - abs(mm_pz)))+tension_v4;
	
	if(dtenz2 < 0){ dtenz2 = 0.06;}

		dtenz1 = 0.00;

	if((tension_v4 < 0.2)&&(dtenz2 < 0.07)){
		dtenz1 = 0.50;
		dtenz2 = 0.00;		
		}
}
