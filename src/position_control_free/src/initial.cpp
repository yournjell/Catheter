...
float target_position_x[5] = {180, 175, 169, 163, 155};
float target_position_y[5] = {208, 214, 224, 231, 238};
float target_position_z[5] = {112, 172, 233, 298, 358};
...
Target_point.color.a = 1.0;
  ...
  Target_point.pose.position.y  = -(((target_position_y[0])-411)/4.0)/10 ; 
  Target_point.pose.position.z  = -(((target_position_z[0]-91))/3.5)/10;//((position_side_final[i][2]/4.08)-(457/4.08))*0.1;
...
Target_point.color.a = 1.0;
  ...
  Target_point.pose.position.y  = -(((target_position_y[1])-411)/4.0)/10 ; 
  Target_point.pose.position.z  = -(((target_position_z[1]-91))/3.5)/10;//((position_side_final[i][2]/4.08)-(457/4.08))*0.1;
...
for(int h = 0; h <5 ;h++){
  ...
  Target_point.pose.position.y  = -(((target_position_y[h])-411)/4)/10 ; 
  Target_point.pose.position.z  = -(((target_position_z[h]-91))/3.5)/10;//((position_side_final[i][2]/4.08)-(457/4.08))*0.1;
...
target_y =-(target_position_y[0]-411)/4.0;
target_z = -(target_position_z[0]-91)/3.5;
...
Target_point.color.a = 1.0;
  ...
  Target_point.pose.position.y  = -(((target_position_y[tnum])-411)/4.0)/10; 
  Target_point.pose.position.z  = -(((target_position_z[tnum]-91))/3.5)/10;//((position_side_final[i][2]/4.08)-(457/4.08))*0.1;
...
target_y =-(target_position_y[tnum]-411)/4.0;
target_z = -(target_position_z[tnum]-91)/3.5;
...
break;
		...
		if(abs(abs(target_y) - abs(mm_py))<=4){
		dteny2 = 7.00;
		dteny1 = 7.00;
		check_y = 1;
		}
		...
		if(mm_py>0){
			...
			if((dteny1 < 0.07 )&&(tension_v1 < 0.08)){ dteny2 = (0.01*abs(abs(target_y) - abs(mm_py)));//(0.01*abs(mm_py- target_y)) ;
								   dteny1 = 0.00; }
		...
		dteny1 = 0.00;
				...
				if ((abs(mm_py-target_y)<0.5)&&((mm_py/target_y)>0)){dteny2 = tension_v2;}
				...
				if((dteny2<0.07)&&(tension_v2 < 0.08)){ dteny1 = (0.01*abs(abs(target_y) - abs(mm_py))) ;
								       dteny2 = 0.00; }
		...
		if(abs(abs(target_z) - abs(mm_pz))<=4){
		...
		check_z = 1;
			...
			dtenz2 = 0.00;
				...
				if((dtenz1 < 0.07 )&&(tension_v3 < 0.08)){ dtenz2 = (0.01*abs(abs(target_z) - abs(mm_pz))) ;
								   dtenz1 = 0.00; }
				...
				check_z = 0;
						...
						if((dtenz2 < 0.07 )&&(tension_v4 < 0.08)){ dtenz1= (0.01*abs(abs(target_z) - abs(mm_pz)));
									 		  dtenz2 = 0.00; }
...
if (dteny1>=5.00){ dteny1 = 5.00;
 dteny1_pre = tension_v1;}
if (dteny2>=5.00){ dteny2 = 5.00;
dteny2_pre = tension_v2;}
if (dtenz1>=5.00){ dtenz1 = 5.00;
dtenz1_pre = tension_v3;}

if (dtenz2>=5.00){ dtenz2 = 5.00;
dtenz2_pre = tension_v4;}
...
//ROS_INFO("check_x: %f,chech_y: %f,check_z: %f", check_x, check_y, check_z );
